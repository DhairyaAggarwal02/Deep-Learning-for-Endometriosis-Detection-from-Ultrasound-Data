...
def make_optimizer(model: nn.Module, phase: str, lr_head: float, lr_backbone: float, weight_decay: float):
    if phase == "head":
        params = [{"params": model.heads.parameters(), "lr": lr_head}]

    elif phase == "layer4+head":
        params = [
            {"params": model.encoder.layers[-1].parameters(), "lr": lr_backbone},
            {"params": model.heads.parameters(), "lr": lr_head},
        ]

        if hasattr(model.encoder, "ln"):
            params.append({"params": model.encoder.ln.parameters(), "lr": lr_backbone})
